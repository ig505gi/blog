...
// Move TOC on mobile: between header and article-body
function repositionTOC() {
  if (!tocSidebar) return;
  const articleWrap = document.querySelector('.article-wrap');
  const articleMain = articleWrap ? articleWrap.querySelector('.article-main') : null;
  const articleHeader = articleMain ? articleMain.querySelector('.article-header') : null;
  const sidebarSlot = articleWrap;

  if (window.innerWidth <= TOC_BREAKPOINT) {
    if (!tocMoved && articleHeader) {
      articleHeader.after(tocSidebar);
      tocSidebar.classList.add('toc-inline');
      tocMoved = true;
    }
  } else {
    if (tocMoved && sidebarSlot) {
      sidebarSlot.appendChild(tocSidebar);
      tocSidebar.classList.remove('toc-inline');
...
repositionTOC();
window.addEventListener('resize', repositionTOC);
